...
void ultrasonic_led_(void){
	int led_count = ultrasonic_distance / 58; // 1단위로 만들기
	if(led_count <= 2){
		*(unsigned char *) 0x3B = 0x01;		
	}
	if(led_count <= 3 && led_count > 2){
		*(unsigned char *) 0x3B = 0x03;
	}
	if(led_count <= 4 && led_count > 3){
		*(unsigned char *) 0x3B = 0x07;
	}
	if(led_count <= 5 && led_count > 4){
		*(unsigned char *) 0x3B = 0x0f;
	}
	if(led_count <= 6 && led_count > 5){
		*(unsigned char *) 0x3B = 0x1f;
	}
	if(led_count <= 7 && led_count > 6){
		*(unsigned char *) 0x3B = 0x3f;
	}
	if(led_count <= 10 && led_count > 7){
		*(unsigned char *) 0x3B = 0x7f;
	}
	if(led_count >= 10){
