...
extern void (*washing_command_queue[])();
...
extern volatile int standby_mode;
...
//짝수 페러티 ,홀수 페러티,
void washing_command(void){
	if(front != rear){	// data가 ru_buff에 존재하는지 check
		printf("%s", rx_buff[front]);	//
		if(strncmp(rx_buff[front],"start", strlen("start")) == 0){
			func_state = 0;
			washing_command_queue[func_state]();
		}else if(strncmp(rx_buff[front],"stop", strlen("stop")) == 0){
			func_state = 1;
			washing_command_queue[func_state]();
		}
