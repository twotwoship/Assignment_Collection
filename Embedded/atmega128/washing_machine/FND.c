...
void fnd_washing_machine(int mechine_time){
	fnd_washing_work(mechine_time);
}
...
void fnd_washing_work(int mechine_time){ // 초단위로 들어옴 1분이상일땐 1분단위로 1분 아래일땐 초단위로
	...
	uint8_t fnd_font[] = {0xc0, 0xf9, 0xa4, 0xb0, 0x99, 0x92, 0x82, 0xd8, 0x80, 0x98, 0x7f};
		...
		switch(digit_select){
			case 0:	//10^0
			FND_DIGIT_PORT = 0x00;
			FND_DATA_PORT = fnd_font[(mechine_time / 60) % 10];
			FND_DIGIT_PORT = 0x80;
			break;
			
			case 1:	//10^1
			FND_DIGIT_PORT = 0x00;
			FND_DATA_PORT = fnd_font[(mechine_time / 600) % 6];
			FND_DIGIT_PORT = 0x40;
			break;
		...
		if(mechine_time < 60){		// 1분 미만 남앗을때
		switch(digit_select){
			case 0:	//10^0
			FND_DIGIT_PORT = 0x00;
			FND_DATA_PORT = fnd_font[mechine_time % 10];
			FND_DIGIT_PORT = 0x80;
			break;
			
			case 1:	//10^1
			FND_DIGIT_PORT = 0x00;
			FND_DATA_PORT = fnd_font[mechine_time / 10 % 6];
			FND_DIGIT_PORT = 0x40;
			break;
...
void fnd_third(uint32_t stopwatch_sec_count, uint32_t stopwatch_ms_count){ // 스탑와치돌아가게하기
	...
	switch(digit_select){
		case 0:	//10^0
		FND_DIGIT_PORT = 0x00;
		FND_DATA_PORT = fnd_font[stopwatch_ms_count /10 % 10];	//ms의 10의자리만 나와야됨.
		FND_DIGIT_PORT = 0x80;
		break;
