...
volatile int top = -1;
volatile int i_top = -1;
volatile int ocb = 0; // open_culry_bracket
volatile int ccb = 0; // close_culry_bracket
volatile uint8_t calc_error = 0;

// 중위 후위 변환
void back_display_make(char* inbuff, char* temp_buff, char* operater_stack) {		// 중위 -> 후위 변환
	int temp_buff_check = 0;
	for (int i = 0; i < MAX; i++) {
		if (inbuff[i] == '=') { // if inbuff[i] is \0, breaking keypad에서는 \0 안들어오니까 =으롣 대체
...
void back_display_carculate(char* temp_buff, int* number_stack) {	
	...
	int temp_num = 0;
	for (int i = 0; i < MAX; i++) {
		if (temp_buff[i] == '=') { // if temp_buff[i] is \0, breaking , 보드에서는 =으로통일
			break;
		}
